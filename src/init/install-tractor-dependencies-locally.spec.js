...
describe('tractor - init/install-tractor-dependencies-locally:', () => {
    ...
    it('should install all of the required dependencies', () => {
        ...
        .then(() => {
            ...
            expect(childProcess.execAsync).to.have.been.calledWith('npm install --save-dev --save-exact tractor-plugin-loader@0.2.2');
        })
        .finally(() => {
            childProcess.exec.restore();
            childProcess.execAsync.restore();
    ...
    it('should tell the user if all dependencies are already installed', () => {
        sinon.stub(childProcess, 'exec').returns({
            stdout: {
                on: (event, callback) => {
                    callback('bluebird@2.10.2 chai@2.3.0 chai-as-promised@5.1.0 cucumber@1.3.1 cucumber-html-reporter@0.3.5 httpbackend@1.2.1 protractor@4.0.11 protractor-cucumber-framework@0.6.0 tractor-plugin-browser@0.1.0 tractor-plugin-loader@0.2.2');
                }
            }
        });
        sinon.stub(childProcess, 'execAsync').returns(Promise.resolve(''));
