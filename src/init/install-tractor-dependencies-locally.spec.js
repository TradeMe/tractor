...
describe('cli/init: install-tractor-dependencies-locally:', () => {
    ...
    it('should tell the user if all dependencies are already installed', () => {
        sinon.stub(childProcess, 'exec').returns({
            stdout: {
                on: (event, callback) => {
                    callback('bluebird@2.10.2 chai@2.3.0 chai-as-promised@5.1.0 cucumber@0.7.0 cucumber-html-report@0.2.5 httpbackend@1.2.1 protractor@2.5.1 tractor-plugin-browser@0.1.0-beta.1 tractor-plugin-loader@0.1.0-beta.4');
                }
            }
        });
        sinon.stub(childProcess, 'execAsync').returns(Promise.resolve(''));
